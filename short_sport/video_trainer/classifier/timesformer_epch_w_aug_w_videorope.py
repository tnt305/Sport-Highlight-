...
class TimeSformer(nn.Module):
    def __init__(self, num_frames, num_classes=18):
        ...
        self.video_rope = VideoRoPE(dim_size= self.backbone.config.hidden_size, patch_size=(16, 16))
    ...
    def forward(self, images):
        ...
        # Áp dụng VideoRoPE trước khi lấy [CLS] token
        x = self.video_rope(x, self.num_frames)  # Rotary Position Embedding
        # Lấy embedding của [CLS] token (vị trí đầu tiên)
        cls_embedding = x[:, 0, :]
        x = self.classifier(cls_embedding)
        return x

class TimeSformerEpochBasedWithAugmentationRoPEExecutor:
