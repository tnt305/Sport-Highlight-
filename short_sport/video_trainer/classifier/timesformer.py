...
class TimeSformer(nn.Module):
    def __init__(self, num_frames, num_classes=10):
        super().__init__()
        self.backbone = TimesformerModel.from_pretrained("facebook/timesformer-base-finetuned-k600", num_frames=num_frames, ignore_mismatched_sizes=True)
...
class TimeSformerExecutor:
    def __init__(self, train_loader, test_loader, criterion, eval_metrics, class_list, test_every, distributed, gpu_id, logger=None) -> None:
        super().__init__()
        self.train_loader = train_loader
        self.test_loader = test_loader
        self.criterion = criterion.to(gpu_id)
        self.eval_metric = {name: metric.to(self.gpu_id) for name, metric in eval_metrics.items()}
